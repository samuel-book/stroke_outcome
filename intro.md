...
The t=NE mRS distributions are based on mRS distribution data when patients did not receive any treatment (this represents what will happen if take the treatment at t=NE). This data is obtained from the untreated control group in clinical trials and adjusted to include the risk of excess deaths caused by taking the treatment.

The two obtained mRS distribution estimates (one for t=0, and one for t=NE) are shown in two different plot formats: a block plot and a line plot. Below we will describe how we derived the two mRS distributions, and how they are represented in each of the charts, for each patient-treatment cohort in turn.

## Patient-treatment cohort 1: nLVO-IVT

This section describes the methods and estimates for the patients with an nLVO that have a IVT. First lets describe how we define the two mRS distributions: t=0 & t=NE.
...
####  mRS distribution estimates as a line plot: Relationship between time to treatment and effect (for nLVO-IVT)

Modelling of the effect of IVT after any given treatment time assumes that the log odds decay uniformly over time between stroke onset and the time to no effect (as modelled by Emberson et al. for IVT). The time at which being given treatment has no-effect is taken as 6.3 hours for IVT (Emberson et al.).
...
This section describes the methods and results for the patients with an LVO that have a IVT. First lets describe how we define the two mRS distributions: t=0 & t=NE.

### mRS distribution at t=0 (for LVO-IVT)

We assume patients treated at t=0 are either fully recovered, or the treatment had no effect. We need a mRS distribution for each of these, and then take a weighted combination (18% fully recovered & 82% no effect), where the weights are chosen to match P(mRS <= 1) of 0.20 which is set as a target by extrapolating the control group mRS for patients with NIHSS 11+ from Emberson et al. 2014 back to a predicted odds ratio of mRS 0-1 of 2.0 at t=0.

The mRS distribution for fully recovered patients is taken as the pre-stroke mRS distribution (this will represent a patient receiving a 100% effective treatment). This distribution comes from the SAMueL dataset, extracting the patients that have an ischaemic stroke and using NIHSS 11+ as a surrogate for LVO. This distribution is then corrected for the excess deaths due to treatment with IVT for LVO patients (3.41%, see appendix).
...
The control population from Goyal et al. 2016. This distribution is then corrected for the excess deaths due to treatment with IVT for LVO patients (3.41%, see appendix).

### mRS distribution estimates as a block plot (for LVO-IVT)

Distributions for the effect of IVT, given either at time of stroke onset or time when the effect of treatment has decayed to zero, are shown in {numref}`figure {number} <LVO_IVT_dist>`. 
...
####  mRS distribution estimates as a line plot: Relationship between time to treatment and effect (for LVO-IVT)
...
This section describes the methods and results for the patients with an LVO that have a MT. First lets describe how we define the two mRS distributions: t=0 & t=NE.

### mRS distribution at t=0 (for LVO-MT)

We assume patients treated at t=0 are either fully recovered*, or the treatment had no effect. We need a mRS distribution for each of these, and then take a weighted combination (75% fully recovered & 25% no effect), where the weights are taken from Hui et al. 2020, who reported 75% successful recanalisation with thrombectomy.

The mRS distribution for fully recovered patients with MT at t=0 is taken as the pre-stroke mRS distribution (this will represent a patient receiving a 100% effective treatment). This distribution comes from the SAMueL dataset, extracting the patients that have an ischaemic stroke and using NIHSS 11+ as a surrogate for LVO. This distribution is then corrected for the excess deaths due to treatment with MT (3.6%, see appendix).
...
The weighted combination of pre-stroke (75%) and untreated LVO at no-effect-time (25%). 
...
The control population from Goyal et al. 2016. This distribution is then corrected for the excess deaths due to treatment with MT (3.6%, see appendix).

### mRS distribution estimates as a block plot (for LVO-MT)
...
####  mRS distribution estimates as a line plot: Relationship between time to treatment and effect (for LVO-MT)
...
## Notes: Excess deaths due to treatment
...
## Extra words DELETE THEM BEFORE GITHUB

We derived six datasets for this work in order to represent the mRS outcome for two different patient cohorts (nLVO and LVO) receiving the available treatments (IVT and MT). If patients receive treatment then the mRS outcome is for them receiving the treatment immediately at time of stroke onset (at t=0). We also derived a dataset to represent the best outcome for all of the patients, this represents each patient receiving a 100% effective treatment. We use these six datasets to fit our relationships. We also show the effect of treatment when this has decayed to zero


The mRS distribution for no effect is not as simple for this case as there is not a large clinical trial for nLVO patients. Instead of taking a mRS distribution for an untreated control group, we will use data from Emberson et al. 2014. 

## Descriptions of the mRS data sets derived
